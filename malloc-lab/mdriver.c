...
extern void mm_heapdump(const char *tag, int opnum, int index, int size);
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges)
{
	...
	for (i = 0; i < trace->num_ops; i++)
	{
		...
		case ALLOC: /* mm_malloc */
			...
			if (verbose > 1 && 4350 < i && i < 4360) mm_heapdump("ALLOC", i, index, size);
			break;
		...
		case REALLOC: /* mm_realloc */
			...
			trace->block_sizes[index] = size;
			if (verbose > 1 && 4350 < i && i < 4360) mm_heapdump("REALLOC", i, index, size);
			break;
		...
		case FREE: /* mm_free */
			...
			mm_free(p);
			if (verbose > 1 && 4350 < i && i < 4360) mm_heapdump("FREE", i, index, 0);
			break;
